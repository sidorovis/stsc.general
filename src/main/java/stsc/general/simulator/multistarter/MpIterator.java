...
/**
 * Named {@link MpIterator}. This is special Iterator concept that implements {@link Iterator} interface with {@link ResetableIterable} extension.<br/>
 * MultiParameter Iterator provide possibility to get values from iterator by index (like c++ random access iterator). Also you can get Random indexed iterator
 * value with {@link MpIterator#getRangom()}.
 * <hr/>
 * Also it is possible to find index by value. You can merge two values from one {@link MpIterator} domen: {@link MpIterator#merge(T left, T right)}.<br/>
 * By merge we understand getting value between left and right (including left and right). <b>Remember</b> result is not stable, so two merge calls with same
 * parameter could return different answers.
 */
public abstract class MpIterator<T, E extends MpIterator<T, E>> implements ResetableIterator<T>, Cloneable {
	...
	private T mergeByIndex(int leftIndex, int rightIndex) {
		int newIndex = 0;
